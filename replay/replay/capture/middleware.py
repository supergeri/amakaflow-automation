...
Usage::
    ...
    from replay.capture import CaptureMiddleware
...
DEFAULT_CAPTURE_POINTS: dict[tuple[str, str], str] = {
    ("POST", "/api/workouts/import/stream"): "web-ingest",
    ("POST", "/api/workouts/save/stream"): "backend-stored",
    ("GET", "/workouts/incoming"): "phone-sync-request",
    ("GET", "/workouts/scheduled"): "phone-sync-request",
    ("POST", "/workouts/completions"): "completion-received",
    # watch-export: endpoint TBD — will be mapped when the watch sync
    # API is finalized. Placeholder uses the likely path.
    ("POST", "/workouts/watch-export"): "watch-export",
}
...
SSE_PATHS: set[str] = {
    ...
    "/api/workouts/bulk-import/stream",
    "/chat/stream",
}
